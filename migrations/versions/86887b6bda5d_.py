"""empty message

Revision ID: 86887b6bda5d
Revises: b134f7d30268
Create Date: 2023-01-26 14:38:36.806828

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '86887b6bda5d'
down_revision = 'b134f7d30268'
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('favoritos',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('agregar', sa.String(length=250), nullable=False),
    sa.Column('eliminar', sa.String(length=250), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('personajes',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('nombre', sa.String(length=250), nullable=False),
    sa.Column('apellido', sa.String(length=250), nullable=False),
    sa.Column('genero', sa.String(length=250), nullable=False),
    sa.Column('color_piel', sa.String(length=250), nullable=False),
    sa.Column('color_pelo', sa.String(length=250), nullable=False),
    sa.Column('color_ojos', sa.String(length=250), nullable=False),
    sa.Column('fecha_nacimiento', sa.String(length=250), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('planetas',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('nombre', sa.String(length=250), nullable=True),
    sa.Column('poblacion', sa.String(length=250), nullable=True),
    sa.Column('terreno', sa.String(length=250), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('vehicles',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('nombre', sa.String(length=250), nullable=True),
    sa.Column('modelo', sa.String(length=250), nullable=True),
    sa.Column('año_creacion', sa.String(length=250), nullable=True),
    sa.Column('capacidad', sa.String(length=250), nullable=False),
    sa.PrimaryKeyConstraint('id')
    )
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_table('vehicles')
    op.drop_table('planetas')
    op.drop_table('personajes')
    op.drop_table('favoritos')
    # ### end Alembic commands ###
